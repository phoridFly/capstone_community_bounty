...
interface propsInterface {
  currentBuyerSPTs: SolicitsPlusTransactions[];
  completedBuyerSPTs: SolicitsPlusTransactions[];
}

export const BuyerView: React.FunctionComponent<propsInterface> = (props: propsInterface) => {
  ...
  const currentTransacs = props.currentBuyerSPTs.map((value:SolicitsPlusTransactions) =>
  (<React.Fragment>
      <Paper key={value._id} className={classes.card} variant="outlined">
        <Grid item>
        <Typography className={classes.seller_nickname}>
            Seller: {value.seller_nickname}
          </Typography>
          ...
          {(value.message_log === undefined || value.message_log === null || messagesToShow !== value._id) ?
            <Button color="secondary" variant="contained" onClick={()=>updateMessagesToShow(value._id!)}>Show Message Log</Button>
            :
              <Typography>
                <Button color="secondary" variant="contained" onClick={()=>updateMessagesToShow('')}>Hide Messages</Button>
              {
                Object.values(value.message_log).map((msgVal:any) => {
                  return (
                    <Container>
                      <Typography>
                        {(msgVal.person_sub === user.sub) ? "You " : "Seller "}
            ...
            if (window.confirm('Are you sure?\nCompleted transactions do not allow for more messages.\nCompletion is irreversible.'))
              ...
              try {
                ...
                updateTransactionStatus({
                  ...
                  user.sub,
                  ("[Buyer marked transaction as complete]")
                );
              } catch (err) {
                console.log(err);
                }
  ...
  const completedTransacs = props.completedBuyerSPTs.map((value:SolicitsPlusTransactions) =>
  (<React.Fragment>
      <Paper key={value._id} className={classes.cardComplete} variant="outlined">
        <Grid item>
        <Typography className={classes.seller_nickname}>
            Seller: {value.seller_nickname} | COMPLETED
          </Typography>
          <Typography className={classes.product_name}>
            Product: {value.product_name} per {value.product_cost}
          </Typography>
          <Typography className={classes.cost_unit}>
            {value.cost_unit}
          </Typography>
          <Typography className={classes.description}>
            Description: {value.description}
